...
export default function MainComponent({ allArticles, handleArticleCLick, getCategorizedNews }) {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", padding: "10px", margin: "auto", height: "calc(100vh - 64px)", boxSizing: "border-box", alignItems:"center", width:"100%" }}>
            <div style={{maxWidth:"1440px", width:"90vw"}}>
                {
                    config.isMobileDevice ?
                        <Carousel width= "100%" height= "50vh" animation="slide" duration={1200}>
                            <ThumbnailComponent handleArticleCLick={handleArticleCLick} article={loadedArticles['headlines'][0]} width={"100%"} height={"100%"} titleClass={"title"} />
                            <ThumbnailComponent handleArticleCLick={handleArticleCLick} article={loadedArticles.headlines[1]} width={"100%"} height={"100%"} titleClass={"title"} />
                            <ThumbnailComponent handleArticleCLick={handleArticleCLick} article={loadedArticles['headlines'][0]} width={"100%"} height={"100%"} titleClass={"title"} />
                            <ThumbnailComponent handleArticleCLick={handleArticleCLick} article={loadedArticles.headlines[2]} width={"100%"} height={"100%"} titleClass={"title-sm"} />
                        </Carousel>
                    :
